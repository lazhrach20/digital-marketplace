...
/**
 * In-process fallback provider (F4/F7). Simulates `POST /issue` without a listen.
 *
 * Uses its own `request_id` (`req_{orderId}-B`) and {@link PROVIDER_B_ERROR_RATE}.
 * Same shared {@link InventoryService} pool and {@link ProviderRequestStore} as A.
 *
 ...
 */
@Injectable()
export class ProviderBAdapter implements IssueProvider {
  ...
  constructor(
    ...
    this.errorRate = parseRate(process.env.PROVIDER_B_ERROR_RATE);
    this.timeoutRate = 0;
